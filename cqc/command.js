...
var commandChannelCfg = function commandChannelCfg(functionName, args) {
  return [[{
    validator: function validator(argument, list) {
      return list && list.length && list.length === 1;
    },
    error: new TypeError("Failed to execute '" + functionName + "': 1 arguments required, but " + args.length + " present.")
  }, {
    validator: function validator(argument) {
...
function command(types, context) {
